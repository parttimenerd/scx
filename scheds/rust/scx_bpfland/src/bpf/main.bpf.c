...
static bool is_task_interactive(struct task_struct *p)
{
	...
	/*
	 * Always consider tasks that haven't ran in the previous slice_ns
	 * period as interactive to boost their priority.
	 */
	return vtime_before(tctx->last_running + slice_ns, bpf_ktime_get_ns());
}
